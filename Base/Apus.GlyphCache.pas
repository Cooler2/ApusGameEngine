...
begin
 ...
 if (width>cacheItemSizes[kind,0]) or
   ...
   raise EWarning.Create('Cache block is too large: '+Conv.ToStr(width)+','+Conv.ToStr(height));
 if kind=0 then
  raise EError.Create('Invalid block type '+Conv.ToStr(width)+'x'+Conv.ToStr(height));
...
begin
 ...
 if hash[h]=block then
  ...
  if h<0 then raise EError.Create('Block is not in hash! '+Conv.ToStr(block));
