...
//  procedure DrawLineParticles(x,y:integer;old,cur:PParticle;count:integer;zDist:single=0); override;
  ...
  // Вывод текста (старый, лучше больше не использовать)
  function PrepareFont(fontNum:integer;border:integer=0):THandle; override; // Подготовить шрифт (из DirectText) к использованию
  procedure FreeFont(font:THandle); override; // Удалить подготовленный шрифт
  procedure SaveToFile(font:THandle;name:string); override; // Сохранить шрифт
  function LoadFontFromFile(name:string):THandle; override; // Загрузить из файла
  procedure SetFont(font:THandle); override; // Выбрать шрифт
  ...
  procedure WriteSimple(x,y:integer;color:cardinal;st:string;align:TTextAlignment=taLeft;spacing:integer=0); override; // Простейший вывод текста
  ...
  // установить таблицу перекодировки / рисовалку спецсимволов
...
var
 ...
 symwidths:array[0..255] of byte; // Ширина символов
 symheight:array[0..255] of byte; // высота символов
 symbase:array[0..255] of byte;   // расстояние от верхней линии до начала символа
 symorder:array[0..255] of byte;  // порядок символов, отсортированный по убыванию их высоты
...
begin
 n:=0;
 try
  if FontNum>100 then dec(FontNum,100); // Поддерживаем как порядковый номер шрифта, так и хэндл
                                        //(В directText - handle=num+100)
  // найти свободный хэндл
  for n:=1 to 32 do
  ...
  // Определить размеры символов шрифта
  ...
  for i:=32 to 255 do begin
   ...
   // Определение начала по вертикали
  ...
  // Занесем в список номера всех символов
  j:=0;
  for i:=0 to 255 do
  ...
  // Определить размер текстуры и выделить ее
...
//  while texh and 3<>0 do inc(texh);
  // Определим подходящий формат текстуры
...
//  Alphamode:=texMan.QueryParams(texw,texh,ipfA8,0);
  ...
  for i:=32 to 255 do begin
   j:=RasterFonts[fontnum].CharImages[i]; // номер символа в DirectText
   with fonts[n].chars[chr(i)] do begin
    ...
    x2:=symcrd[j,2];
    y2:=symcrd[j,3];
    if chr(i) in ['A'..'Z','a'..'z','А'..'Я','а'..'я','0'..'9','(',')','[',']','\','/','<','>','?',',','.'] then begin
 ...
 try
  ...
  try
   ...
   for i:=32 to 255 do begin
    ...
    blockread(f,fonts[n].chars[chr(i)].flags,1); // резерв
...
begin
 ...
 for i:=tex.height-1 downto 0 do begin
  pb:=tex.data; inc(pb,tex.pitch*i);
  if tex.PixelFormat in [ipf8Bit,ipfA8] then   // дальше непонятный код!
   for j:=0 to tex.width-1 do begin
 ...
 for i:=32 to 255 do begin
  ...
  blockwrite(f,fonts[font].chars[chr(i)].flags,1); // резерв
...
begin
 ...
 if unicode then begin
  ...
  // если строка не была переведена, либо словарь не в юникоде
...
begin
 ...
 translated:=false;
 st:=translate(st);
 // если строка была переведена и словарь в юникоде
 if unicode then begin
  // строка в юникоде
  if fontMatch[curfont]>0 then begin
   TextOut(fontMatch[curfont],x,y+fontMatchAddY[curFont],color,DecodeUTF8(st),align,toAddBaseline,spacing);
   exit;
  end else // если замены шрифта нет - перевести строку в 8-бит кодировку
   st:=UTF8Decode(st);
 end else
  // если строка не была переведена, либо словарь не в юникоде
  if fontMatch[curfont]>0 then begin
...
// dec(y,fonts[curfont].baseAdd div 2);
 ...
 if (align=taJustify) and not (st[length(st)] in [#10,#13]) then begin
  cnt:=0;  // посчитаем кол-во пробелов
  skipflag:=false;
  ...
  i:=GetTextWidth(st);
  if i<round(spacing*0.95-10) then AddSpace:=0  // дополнительное пр-во для выравнивания
  ...
  for i:=1 to length(st) do
   if not skipflag and (st[i]<>#1) then begin
    ...
    if st[i]=#1 then begin skipflag:=true;
     // отрисовка вставленной картинки
...
begin
 ...
 ASSERT(curfont in [1..32]);
// inc(x,_ofsX); inc(y,_ofsY); - не нужно! все выводится методами, которые и так добавляют смещение
...
// {$ENDIF}
 ...
 translated:=false;
 st:=translate(st);
 if fontMatch[curfont]>0 then begin // если есть юникодовый шрифт - использовать его
  if unicode or IsUTF8(st) then // строка изначально в юникоде
   wst:=UTF8Decode(st)
  else
   wst:=st;  // иначе - преобразовать строку в 8-bit
 end else begin
  if unicode then begin
   // строка в юникоде (UTF8)
   if fontMatch[curfont]>0 then begin
    wst:=UTF8Decode(st)
   end else
    st:=UTF8Decode(st);
...
// dec(y,fonts[curfont].baseAdd div 2);
 ...
 // Alignment
 sp:=0; AddSpace:=0;
 // длина строки в зависимости от её типа (в виртуальных пикселях)
 if wst<>'' then
 ...
 // Временно раскомменчено для PC, было закомменчено для iOS
{ ew:=0; eh:=0;
 ...
 if (ew<>0) or (eh<>0) or (efftex.scaleX<>scaleX) or (efftex.scaleY<>scaleY) then begin}
  ...
  begin
   ...
   eh:=GetPow2(eh);
   {$ENDIF}
   // TODO: Нужно поискать подходящую текстуру в кэше, и если такая есть - использовать её
...
// FillRect(x,y,x+width,y+width,$FF00F000);
 ...
 // Этап 1: нарисовать текст в маску
 // ----------------------------------
 saveX:=x;
 px:=0;
 interval:=1;
// a:=color shr 24; // alpha
 ...
 efftex.lock;
 // Этап - врисовать текст в текстуру эффекта
