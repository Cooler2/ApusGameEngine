...
| R-24 | Android Platform Revival | in-progress | ~45% | TouchDemo builds/runs on ARM64 emulator with GLES 3, packaged PNG asset, line rendering, and primary-touch drag; next: safe areas, lifecycle/context restore, text input, clean exit |
...
- Done, S1 (2026-07-12): TouchDemo runs on the API 36 ARM64 emulator through Pascal `SDL_main`, creates an accepted GLES 3.0 context (GLES 3.1 exposed), renders the line diagnostic and packaged PNG sprite at the native `720x1280` / 320 dpi smoke-profile size, and accepts primary-touch drag through SDL's unified pointer path. Android SDL sizing no longer overwrites the native surface with desktop-configured dimensions.
- Next: system-bar/display-cutout safe area, pause/resume and GLES context restoration, clean exit, text input, then CI compile/package/runtime gates.
