...
| R-24 | Android Platform Revival | in-progress | ~50% | TouchDemo runs immersive fullscreen on ARM64/GLES 3 with packaged PNG, stable redraw, primary-touch drag, and important logcat output; next: lifecycle/context restore, text input, clean exit |
...
- Done: TStyleBlock, resolver, @refs, state blocks, patch, transitions, draw migration, StyleDemo, TStyleCatalog; font/color/styleClass removed from TUIElement; canonical key vocabulary + selective inheritance (`IsInheritedStyleKey`, 2026-09-02, driven by port blocker B-16); skinned widgets through the common box path (`background-image/tint/size/offset-x/y`, buttons and windows on `DrawCommonStyle`, `:focused` state, `TUIWindow.background` removed, 2026-09-05); content keys (`text-align/shadow/decoration/offset-x/y`) + state blocks inside named styles fixed (refs+tokens, 2026-09-05).
- Remaining:
...
- Done, S1 (2026-07-12): TouchDemo runs on the API 36 ARM64 emulator through Pascal `SDL_main`, creates an accepted GLES 3.0 context, and renders the line diagnostic and packaged PNG sprite at the native `720x1280` / 320 dpi smoke-profile size. SDL's unified pointer path handles primary-touch drag; immersive sticky mode covers the entire display; continuous demo redraw avoids relying on undefined mobile backbuffer preservation; `Apus.Log` mirrors `Forced+` messages to logcat under `ApusEngine`.
- Next: pause/resume and GLES context restoration, clean exit, text input, then CI compile/package/runtime gates. Add safe-area placement when TouchDemo gains important edge UI.
- Out of scope for MVP: iOS revival, store packaging polish, push, background services, full gamepad coverage, Android UI skins.
- Acceptance Criteria:
  - [x] A minimal Engine5 demo builds for Android64 from documented steps. *(TouchDemo, arm64-v8a debug APK.)*
  - [ ] The demo starts, creates a GLES context, renders a textured/UI scene, and exits cleanly.
  - [x] Assets load from APK/package resources and writable app storage is documented. *(`sprite.png` is copied through `Apus.Android` into app-private `app_Data`.)*
...
- Level-1 gate: DONE on Win64 (2026-08-24) — `SDLMIX` links the SDL2_mixer backend, `Apus.Engine.SoundSDL` reworked (pooled channel objects instead of a per-play leak, relative volumes reapplied on global change, real decoder/device diagnostics, `Pause`), T1 core fixes in `Apus.Engine.Sound` (atomic cross-thread flags, MP3 header parsing + `SampleRateOf` divisor guard, honest backend-selection errors, `ISoundLib.Pause` wired to `SOUND\Pause|Resume`), `soundDemo` migrated to the new foundation API with a non-interactive script mode. Verified: samples, ogg/mp3 music, MOD, volume, pan, pause/resume, fade-out — audible on Win64 (author sign-off), x64+x86 builds, headless run on the SDL `dummy` driver. Two side finds fixed: the vendored `sdl2_mixer.pas` declares the C macros `Mix_LoadWAV`/`Mix_PlayChannel` as exported functions (they are not — the backend calls `Mix_LoadWAV_RW`/`Mix_PlayChannelTimed` instead), and `Apus.EventMan` cut the `event::tag` suffix one character short.
- Backend linkage (2026-08-31, author decision): **audio is opt-in, nothing is linked by default**. `defines.inc` no longer enables any backend on its own; a project that wants sound builds with `-dSDLMIX` (or `-dIMX` for the legacy path), and with no backend compiled in `InitSoundSystem` logs one line and stays inactive instead of raising. Rationale: a backend binding is a load-time import, so linking it makes every executable depend on `SDL2_mixer.dll` / `libSDL2_mixer` — a project that has no sound must not pay for that. The `NOAUDIO` opt-out is gone (there is nothing to opt out of); CI and `demo/build_demo_fpc.cmd` pass `-dSDLMIX` for `SoundDemo` explicitly.
...
- Acceptance Criteria:
  ...
  - [x] Distribution path documented: storefront (no notarization) vs Developer ID + notarization (when a payment path exists — not a blocker for closing the task) — see `platform/BUILDING_BUNDLES.md`.
...
- Status: stages A+B done (2026-08-25) — the public contract is in place | Priority: **P1 — schedule blocker** | Area: Platform / Render / API
- Next step: the author's manual sign-off on Alt+Enter, a live DPI change (drag between monitors) and the MultiWindow demo; then **stage C** (Setup* presets already exist — left: `FitPreviewWindow`, `-PREVIEW`/`-CLIENTSIZE`/`-RENDERSCALE`/`-SAFEAREA`/`-UISCALE`, orientation validation). D (mobile safe area + packaging) goes with the next mobile iteration, E (present shader, integerScale blit) on demand. Detail and the per-stage record: `Work/R-31_api_design.md` §8.
