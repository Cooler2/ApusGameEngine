...
- Scope (MVP):
  ...
  - **Solid is the primary mode** (rev.2 2026-06-15 — **GPU meshes, not immediate triangles**): unit shapes generated by R-20 `MeshShapes`, uploaded once into cached `TGpuMesh` (R-19), drawn per-shape via `transform.SetObj(matrix)` + `gpuMesh.Draw`. Set: `Arrow3D` (cylinder shaft + cone head), `Axes` (3 solid arrows, X=red/Y=green/Z=blue), `Box` (solid AABB), `Sphere` (UV-tessellated), `Capsule` (cylinder + 2 hemispheres).
  ...
  - **Immediate lifetime**: a `SetupRender`/`Flush` session draws in the current frame; no retained queue / no render-loop hook (a retained+duration mode is a possible later extension, not built now).
  - **No batching in this card — relies on R-09.** Solids draw as one `gpuMesh.Draw` (= one `DrawMesh`) per shape; lines draw via plain `draw.Line3D` (one draw call per primitive — acceptable for debug, not a hot path). Efficient 3D line/triangle batching is a non-trivial drawer-level facility **deferred to R-09 (Track C / auto `Batching(on/off)`)**. `SetupRender`/`Flush` are the seam where R-09's `draw.Batching(true/false)` will later slot in. `TDrawer` is **not** modified by this card.
  - **Lighting via the stock mesh shader** (rev.2 2026-06-15 — supersedes "baked into vertex color"): cached unit meshes are white (pos+normal only); the module sets its own `ambient + directional` light (`shader.AmbientLight`/`DirectLight`, configurable via `SetLight`) and the stock mesh shader lights them — world-stable under rotation, no per-frame CPU shading. **Per-primitive colour via a material-tint uniform**: this card implements `shader.Material`'s colour (currently a stub) as a `materialColor` uniform in the *mesh* shader path only (painter untouched); one cached white mesh per shape, tinted per draw (red arrow / blue box = same meshes, different tint). `Flush` must restore `materialColor` to white (global state, like the lights).
  - **Showcase/test surface folded into `demo/MeshLab`** (decided 2026-06-15 — no separate `demo/DebugDraw`): axes, grid, `BoxWire` around the loaded mesh's AABB, plus a block exercising the rest (arrows 2D/3D, box, sphere, capsule) under a rotating camera, with a baked-light-direction toggle — doubles as the manual test surface and visual reference. Other demos pick up shapes in place as natural consumers.
- Out of scope: 3D line/triangle batching in `TDrawer` (deferred to R-09); retained debug-draw queue with per-shape duration; oriented (matrix) box `BoxM`; translucent/sorted solids; specular/shininess in `shader.Material` (only the colour/tint component is implemented here). NB rev.2: cached GPU meshes and shader-based lighting are now **in scope** (the immediate/baked plan was reversed).
- Dependencies (rev.2): `Apus.Engine.MeshShapes` (R-20, solid geometry) + `Apus.Engine.GpuMesh`/`Mesh3D` (R-19, upload/draw) — now **hard deps** for the solid set; `Apus.Engine.ShadersGL` (the `materialColor` tint uniform lands here); `Apus.Engine.Draw` (`draw.Line3D` for the line set), `Apus.Engine.API` (`draw`/`shader`/`transform`/`gfx`), `Apus.Geom3D`. Soft dep on R-09 for batching (functionally complete without it).
- Risks: touching the shared shader generator (mitigated: tint uniform is added to the *mesh* path only, painter untouched, default white = no-op; verify MeshLab unchanged); `materialColor` is global shader state that `Flush` must reset; line set under module light state (lit shader branch needs a normal — lines have none; verify `draw.Line3D` path is unaffected); GPU meshes are context-bound (multi-window deferred); keeping depth/overlay behavior matching the current `DrawGrid`.
- Acceptance Criteria:
  ...
  - [ ] Solid shapes render as cached `TGpuMesh` (R-20 geometry → R-19 upload), drawn via `SetObj(matrix)` + `gpuMesh.Draw`; readable in 3D via the stock mesh shader's `ambient + directional` light (configurable via `SetLight`).
  - [ ] Per-primitive colour works via the `materialColor` tint uniform (`shader.Material` colour implemented, mesh-path only); painter/2D unchanged, MeshLab's colored cube still correct.
  - [ ] `demo/NormalMap` `DrawGrid` is ported onto `DebugDraw` with equivalent visuals (visual-parity validation).
  - [ ] `demo/MeshLab` showcases every primitive (axes/grid/BoxWire + the rest) and builds in the demo flow.
  - [ ] Module compiles under FPC (and Delphi) on Win/Linux.
- Notes:
  - Decisions + rationale: `Work/reports/R-18_debug_shapes.md` (renamed from `R-06_task_debug_shapes.md` 2026-06-15; **R-18 ≠ R-06** — separate cards). Step-by-step **implementation plan** (signatures, T1–T6 checklist): `Work/reports/R-18_implementation_plan.md`.
  - Cross-link: batching of `draw.Line3D`/`draw.Triangle` belongs to **R-09 Track C** (auto `Batching(on/off)` facility). R-18 leaves `SetupRender`/`Flush` as the integration seam so it picks up batching transparently once R-09 ships it.
  - Staged plan (rev.2): (1) implement `materialColor` tint uniform in the mesh shader path + module scaffold + session + light → (2) line primitives (immediate `draw.Line3D`) → (3) solid core: unit shapes from R-20 `MeshShapes` → cached `TGpuMesh` (white, pos+normal), per-shape `Material(color)` + `SetObj(matrix)` + `gpuMesh.Draw` → (4) showcase folded into `demo/MeshLab` + port NormalMap `DrawGrid` → (5) FPC smoke test.
  ...
  - **2026-06-15: UNBLOCKED** (R-19 + R-20 done). Geometry source = R-20 `MeshShapes` (Box; Cylinder/cone=`Cylinder(r,0,h)`; UVSphere/Octasphere, hemisphere=`Octasphere(Hemisphere)`); composites Arrow3D/Axes/Capsule = several cached unit meshes drawn with their own matrices.
  - **2026-06-15 rev.2 (course correction):** the original "immediate — per-frame CPU walk + bake shade into vertex color + emit `draw.Triangle`, no `TGpuMesh`/VBO" plan was **reversed by the author** (the running agent was implementing it). It ignored the ready mesh base R-19/R-20 was built for. New path: solids = cached `TGpuMesh` drawn via `SetObj(matrix)`+`Draw`; lighting via the stock mesh shader; per-primitive colour via a new `materialColor` tint uniform (implements `shader.Material` colour, mesh-path only — also closes the R-17-flagged `Material`-is-a-no-op white spot). Lines stay immediate. Details in `R-18_implementation_plan.md` rev.2 / `R-18_debug_shapes.md` rev.2.
...
- Scope (MVP — LOCKED, full spec in `Work/reports/R-20_meshshapes_design.md`):
  - New module `Apus.Engine.MeshShapes.pas` ({$SCOPEDENUMS ON}), a record-namespace of generator functions, each a **pure fresh producer** returning a new `TMesh` (`Apus.Engine.Mesh3D`). No GPU, no materials, no lighting — pure CPU geometry.
