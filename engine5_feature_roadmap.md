...
- Design DECISION (LOCKED 2026-06-12, consensus Claude+Codex+author; full spec `Work/reports/R-19_mesh_design.md`): (1) **vertex storage** — CPU is **SoA typed arrays** (named per-attribute arrays + optional custom list), interleaving is an upload concern, NOT a storage philosophy; layout is a **GPU-only** descriptor (CPU mesh has no layout). (2) **class structure** — `TMesh` (CPU SoA, triangle-list only) + `TGpuLayout` (immutable, interned by content-hash, compared by id) + `TGpuMesh` (explicit retained upload = gather+format-encode, per-stream VBO, uint16/uint32 IBO chosen at upload) + `TModel`/`TModelInstance` (skeleton+anim+materialSet[section] on top). (3) **layout encoding** — open descriptor `(semantic,format,stream,offset)` with **stable semantic→location table** (location derived, not stored); shader-gen + backend attribute-enable change together as an obligatory pair. (4) **skinning** — GPU is the target (bind-pose uploaded once, joints/weights as attributes); CPU skin stays a valid fallback/reference (its dynamic-stream upload is the remaining justification for static/dynamic split). Also: parts split into geometric `TMeshSection` (range+name, in mesh) vs material set (in model/instance); CPU indices always `array of integer`; instancing first-class (attribute- or buffer-based); generators/ops in separate modules (`Apus.Engine.MeshShapes`/`MeshOps`).
...
- Scope — staged:
  ...
  - **B1:** `TGpuLayout` (interned content-hashed descriptor) + stable semantic→location table. CPU-only.
  - **B2:** `TMesh` (SoA container, triangle-list) + build-API + sections + int32 indices. CPU-only. **Gate: headless unit-test** (offsets/stride/hash/interning, round-trip Set/Get, index validation) before any GL — de-risks B3.
  - **B3:** OpenGL attribute binding by descriptor — obligatory **pair**: shader-gen reads locations from the table + `SetupAttributes` enables a *set* of locations (not contiguous prefix). Stable semantic locations.
  - **B4:** `TGpuMesh` — upload (gather+format-encode), per-stream VBO, IBO width chosen at upload, `UploadDirty`, `muDiscardCPUCopy`.
  - **C1:** sections + 32-bit indices + AEM loader target shape.
  - **C2:** `TModel`/`TModelInstance` on `TMesh`; CPU skin via dynamic stream.
  - **C3:** GPU skinning as render-mode (layout already carries joints/weights); instancing (attribute- and buffer-based).
...
- Notes:
  ...
  - 2026-06-12: **A→B design decision TAKEN** (Claude+Codex+author consensus): CPU SoA, GPU-only interned layout, `TMesh`=triangle-list, sections-without-material, int32 CPU indices, GPU-skin target + CPU fallback, instancing first-class. Etap B split into B1–B4, C into C1–C3. Open non-blockers: bone-matrix delivery in GL, context-loss reload on `muDiscardCPUCopy`, painter-layout coexistence (old packed `TVertexLayout` also serves 2D immediate path).
