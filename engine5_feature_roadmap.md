...
- Done, S1 (2026-07-12): TouchDemo runs on the API 36 ARM64 emulator through Pascal `SDL_main`, creates an accepted GLES 3.0 context (GLES 3.1 exposed), renders the line diagnostic and packaged PNG sprite at the native `320x480` drawable size, and accepts primary-touch drag through SDL's unified pointer path. Android SDL sizing no longer overwrites the native surface with desktop-configured dimensions.
