...
- Status: implemented (runtime sign-off pending) | Priority: P1 | Area: Core Runtime / Input
- **IMPLEMENTED 2026-06-19** (branch `engine5`). Compiles clean x64+x86 (full demo graph). What shipped, vs the plan below:
  - `TGameScene` (Apus.Engine.Scene.pas): split the key buffer into **`keyBuffer`** (down/up → dispatched) + **`charBuffer`** (text → polled by `ReadKey`); added `onKeyDown`/`onKeyUp` final-receiver virtuals + the `DispatchKey` dispatcher (which owns the `RegisterHotKey` table, on press), `RegisterHotKey`/`UnregisterHotKeys`, and `PumpInput(shift)`. `WriteKey` now packs `keyCode|scancode<<16|pressed<<24` (key-up included); new `WriteChar` feeds the char buffer. `SetStatus` clears buffers when a scene leaves active (no stale dispatch).
  - `TGame.KeyPressed` (Game.pas): writes press **and** release to the buffer, keeps `HandleInternalHotkeys`+`keyState`; **removed** `Signal('SCENE\..\KeyDown/KeyUp')`. `CharEntered` → `WriteChar`.
  - `TWindow.OnFrame` (Window.pas): drains every active scene's buffer via `PumpInput(shiftState)` **before** `ProcessScenes` (the single Process chokepoint, so it covers both frame-loop paths). Iterating active scenes is safe because only the kbd-topmost scene ever has buffered keys.
  - `TUIScene.DispatchKey` (UIScene.pas): absorbs the old `KbdEventHandler` logic (focus `onKey` → `ProcessHotKey` → consume), then falls through to `inherited` (scene hotkey table + `onKeyDown`/`onKeyUp`) **only when no element holds focus** — this is the chosen gate that stops a gameplay `[C]` from firing while typing into a (modal) field. `SetEventHandler('Kbd',...)` removed; `WriteKey` override replaced by `WriteChar` (routes char to focus + buffers for polling).
  - `ProcessHotKey` (UITypes.pas) now returns `boolean` (consumed).
  - `MessageScene`: dropped the `Scene\MessageScene\KEYDOWN` subscription; Enter/Escape now via button `SetHotKey` (modal/visibility-aware through `ProcessHotKey`).
  - Demos: **NormalMap** showcases `RegisterHotKey` (D1/D2/D3/N/C → handler methods); **MeshLab** overrides `onKeyDown` (arrows/Tab/B/L case); **InputDemo** event counters moved to `onKeyDown`/`onKeyUp` (char log still via `ReadKey`/charBuffer). All three drop the `SetEventHandler`/`WordFromTag`/`RemoveEventHandler` plumbing.
  - **Resolved open questions:** the catch-all is the `onKeyDown`/`onKeyUp` **final receivers** (symmetric with `onMouseDown`/`onMouseUp`) — the `RegisterHotKey` table lives in the `DispatchKey` **dispatcher**, so overriding a receiver can't bypass it (`onXXX` = endpoint, not dispatcher); **no** separate `onChar` hook this pass (char keeps the `charBuffer`+focus `onUniChar` path); `RegisterHotKey` reuses UI's shift-mask semantics (`reg=shift` or `reg>0 and reg&shift=reg`).
  - **Not touched (deliberate):** `ConsoleScene`/`TweakScene` still subscribe to raw `KBD\KeyDown` — they are global debug overlays in the same category as `HandleInternalHotkeys` (always-on, scene-independent), out of scope like UI `SetHotKey` (architecture-review #6).
  - **Pending:** runtime/visual sign-off (input deferred ~1 frame now — frame-synced like mouse; verify UI typing, MessageScene Enter/Escape, NormalMap/MeshLab keys, InputDemo polling-vs-callback parity). NOT committed yet.
- Priority: P1 | Area: Core Runtime / Input
- Value: Fixes a real correctness bug and removes a confusing parallel-path. Today keyboard input is delivered through **two independent `KBD\*` event subscribers that don't know about each other** — `TGame.onKbdEvent` (→ per-scene buffer + secondary `SCENE\<name>\KeyDown` signal + internal hotkeys) and `TUIScene.KbdEventHandler` (→ focused element / `ProcessHotKey`). Because they run in parallel, a gameplay hotkey (e.g. `[C]`) fires **even while the user is typing into a focused/modal text field**. The fix collapses both into one ordered pipeline.
- Root-cause surfaced during the R-06 NormalMap demo audit (gap-doc п.8): demos hand-roll `SetEventHandler('SCENE\..\KEYDOWN')` + `WordFromTag` + a global scene ref + manual `RemoveEventHandler`. There is no clean virtual `onKeyDown` seam symmetric to `onMouseMove`/`onMouseWheel`.
- **Author decision (2026-06-19): synchronous callbacks replace key signals.** The only real advantage of the signal was async handling (key → save file / network); that is now explicit — a callback calls `Threads`/`RunAsync` or `Signal` itself when it wants async. Synchronous callbacks give deterministic ordering and far easier debugging, with no teleporting control flow through the event bus.
- Target architecture:
  - `KBD\*` stays as the **single platform ingress**, consumed **only** by `TGame`, which writes to the kbd-topmost scene's key buffer (the existing per-scene buffer is the one delivery channel).
  - **`PumpInput`** — a dedicated engine step run once per frame (next to `FlushMouseInput`, **before** `Process`), drains the buffer and calls `DispatchKey` per entry. NOT inside `Process`: `Process` is tied to scene `frequency` and can run slower or faster than render — input dispatch must be render-synced.
  - **`DispatchKey(key;scancode;shift;pressed):boolean`** — the single virtual routing **dispatcher**. Base `TGameScene`: `RegisterHotKey` table (on press) → `onKeyDown`/`onKeyUp` final receivers. `TUIScene` overrides: focused element / `ProcessHotKey` first, unhandled → `inherited` (gameplay). One ordered pipeline: **UI focus → hotkeys → gameplay**.
  - **`TGameScene.RegisterHotKey(key,shift,handler)`** — declarative scene-level key→action binding (the transparent mechanism that works on plain scenes and extends on UI scenes). Reuses modal/enabled/consumption semantics conceptually from UI `ProcessHotKey`.
  - Remove the secondary `SCENE\<name>\KeyDown`/`KeyUp` signal and `TUIScene`'s parallel `SetEventHandler('Kbd',...)`.
- Required buffer change: `WriteKey` is currently called only on press (+char); **key-up is not buffered** (it only exists in the signal, Game.pas:494). To feed `onKeyUp` from the buffer, encode release events too via a free bit in the packed `cardinal`.
- Migration: `InputDemo` (demonstrates the signal path → rework to callbacks, or showcase "Signal from a callback" as a deliberate idiom), `NormalMap`/`MeshLab` (drop `SetEventHandler`/`RemoveEventHandler` → `RegisterHotKey`/`onUnhandledKey`), `UIScene` (move `Kbd` handler logic into `DispatchKey`), `TGame.KeyPressed` (drop the `SCENE\..` signals).
- Keep: `ReadKey`/`KeyPressed` manual polling (InputDemo uses it; orthogonal channel). `HandleInternalHotkeys` (engine hotkeys, §6.1) and UI `SetHotKey` (§6.2) stay as-is — their full unification into one action-map is **architecture-review item #6 (input action mapping)**, a separate future card.
- Open design questions (settle in detailed plan): names (`onUnhandledKey` Godot-style vs `onGameKey`/`onAction`); whether a paired `onChar`/text-input hook ships in the same pass or later; exact `RegisterHotKey` shiftstate-matching semantics (reuse UI's mask logic).
- Design doc: `Work/reports/scene_onkeydown_design.md` (full analysis: current flow, other engines, hotkey landscape, decisions §10).
