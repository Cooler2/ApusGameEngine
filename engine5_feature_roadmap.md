...
- Status: in progress (design locked 2026-06-12; B1+B2 DONE, headless gate passed)
- Priority: P1
- Area: Render / 3D Core
...
- Scope — staged:
  ...
  - **B4:** `TGpuMesh` — upload (gather+encode), per-stream VBO, IBO width chosen at upload, `UploadDirty`, `muDiscardCPUCopy`.
  - **C1:** sections + 32-bit indices + AEM loader target shape.
  - **C2:** `TModel`/`TModelInstance` on `TMesh`; CPU skin via dynamic stream. Rename `Apus.Engine.Mesh3D`→`Apus.Engine.Mesh`, migrate legacy consumers, remove old interleaved unit.
  - **C3:** GPU skinning as a render-mode (layout already carries joints/weights); instancing (attribute- and buffer-based).
...
- Notes:
  ...
  - 2026-06-12: A→B boundary decision taken — full design locked (see `R-19_mesh_design.md`). B1 (`TGpuLayout`+location table) and B2 (SoA `TMesh`+`TBox3`) implemented and committed on `feature/r-19-mesh`, both with passing headless tests; wired into `linux_smoke.sh`. New SoA mesh lives in transitional `Apus.Engine.Mesh3D` (new unit chosen over in-place replacement to keep B headless and not break the ~8 legacy `TMesh` consumers; migrated + renamed in C2). Next: B3 GL-binding pair — needs the §15 painter-layout coexist-vs-migrate call before touching the shared hot path.
